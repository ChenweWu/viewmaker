...
from src.models import resnet_1d
...
from src.models import viewmaker_1d
...
class PretrainExpertInstDiscSystem(pl.LightningModule):
    ...
    def create_encoder(self):
        if self.config.model_params.resnet:
            encoder_model = resnet_1d.ResNet18(
                self.config.model_params.out_dim,
                num_channels=12, # 12 feature spectrograms
                input_size=1,
            )
        else:
            ...
            encoder_model.conv1 = nn.Conv1d(1, 64, kernel_size=7, stride=2,
...
class PretrainViewMakerSystem(PretrainExpertSimCLRSystem):
    ...
    def create_viewmaker(self):
        view_model = viewmaker_1d.Viewmaker(
            num_channels=12,
    ...
    def __init__(self, config):
        ...
        if resnet == 'resnet18':
            if self.config.model_params.use_prepool:
                if self.pretrain_config.model_params.resnet:
        ...
        if not self.pretrain_config.model_params.resnet:
            if self.config.model_params.use_prepool:
                cut_ix = -2
            else:
                cut_ix = -1
